...
7. Wait for the virtual machine to get deploy, deployment may take 20 minutes, once deploy click on **Go to resource group**.
...
12. In the Windows Security window, select More choices and then Use a different account. Enter the Usename as `.\student` and password `Password.1!!` for an account on the virtual machine and then select **OK**.
